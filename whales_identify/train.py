...
def run_training(df_train: pd.DataFrame, img_dir: str, checkpoint_dir: str):
    ...
    train_labels = {
        row["file_path"]: row["individual_id"] for index, row in df_train.iterrows()
    }

    train_dataset = WhaleDataset(
        img_dir=img_dir, labels=train_labels, transform=data_transforms["train"]
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Путь к директории с изображениями для тренировки",
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=str,

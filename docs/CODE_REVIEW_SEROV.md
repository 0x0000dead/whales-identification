...
Полный перечень предложений и улучшений, внесённых в ходе код-ревью, с привязкой к конкретным PR и коммитам.
...
### 1.1 Перевод сравнительного скрипта на реальный расчёт метрик

**Коммит:** `e679c91`  
**PR:** #25  
**Описание:** сравнительный скрипт `comparison_research/comparison_detection_algo.py` приведён к расчёту метрик на реальных изображениях.  
**Реализация:** метрики вычисляются через `scripts/compute_metrics.py` на фиксированном тестовом срезе `data/test_split/`.  
**Обоснование:** все публикуемые метрики вычисляются на фиксированном тестовом срезе (`data/test_split/`) и воспроизводимы.

...
### Результаты измерений production-модели

Все приведённые ниже числа — воспроизводимый вывод `python scripts/compute_metrics.py`
на тестовой выборке `data/test_split/manifest.csv` (100 позитивных + 102 негативных
изображения). Полные JSON и Markdown — `reports/metrics_latest.json`
и `reports/METRICS.md`, автоматически инъектируются в `MODEL_CARD.md`.

| Показатель | EfficientNet-B4 ArcFace (production) | Метод измерения |
|------------|--------------------------------------|-----------------|
| **Anti-fraud gate — Precision (бинарная, «кит/не кит»)** | **0.9048** | `compute_metrics.py` → `anti_fraud.precision` |
| Anti-fraud — Recall / TPR | 0.950 | то же → `anti_fraud.tpr` |
| Anti-fraud — Specificity / TNR | 0.902 | то же → `anti_fraud.tnr` |
| Anti-fraud — F1 | 0.9268 | то же → `anti_fraud.f1` |
| ROC-AUC (cetacean_score) | 0.984 | `_approx_roc_auc` |
| **Species top-1 (на принятых gate изображениях)** | **0.3579** | `identification.species_top1_accuracy` |
| **Species precision (high-confidence, ≥0.10)** | **0.5294** | `identification.species_precision_confident` |
| Individual top-1 (13 837 классов) | 0.22 | `identification.top1_accuracy` |
| Individual top-5 | 0.25 | `identification.top5_accuracy` |
| Latency p50 / p95 / p99 (мс, CPU, 512×512) | 174 / 299 / 417 | `performance` |
| Устойчивость к шуму | max drop −1.1 % | `scripts/benchmark_noise.py` + `reports/NOISE_ROBUSTNESS.md` |
| Масштабируемость (R²) | 1.000 | `scripts/benchmark_scalability.py` + `reports/SCALABILITY.md` |

> **Как читать таблицу.** Anti-fraud Precision 0.9048 — это качество бинарного
> CLIP-гейта («содержит ли фото кита или дельфина»). Species precision 0.5294 —
> качество идентификации вида на изображениях, принятых гейтом, с порогом
> уверенности ≥ 0.10; на high-confidence решениях модель в 53 % случаев
> правильно называет вид. Individual top-1 22 % — вероятность точного совпадения
> конкретной особи среди 13 837 известных индивидов (материально более сложная
> задача, чем распознавание вида). Подробное обоснование и ограничения текущего
> чекпоинта — в `DOCS/PERFORMANCE_REPORT.md` и `reports/METRICS.md`.

Сравнение с альтернативными архитектурами, которые рассматривались на этапе
исследований (ResNet, Swin, ViT-L/32), вынесено в `DOCS/ML_ARCHITECTURE.md` —
там приводится обоснование выбора EfficientNet-B4 ArcFace и ссылки на
ноутбуки `research/notebooks/06_benchmark_multiclass.ipynb` и
`08_benchmark_all_compare.ipynb`, где эти архитектуры сравниваются между собой
на идентичной выборке.
